...
```bash
./.ai/scripts/bootstrap-template.sh "my project"
export HARNESS_SMOKE_COMMAND="./gradlew test bootJar"
...
- `./.ai/scripts/bootstrap-template.sh "My Project"`
  여기의 `"My Project"`는 예시입니다. 실제 프로젝트 이름으로 바꿔야 합니다.
- `export HARNESS_SMOKE_COMMAND="npm run lint && npm test"`
  이것도 예시입니다. 프로젝트의 실제 smoke command로 바꿔야 합니다.
...
프로젝트용 smoke command는 변경을 빠르게 신뢰할 수 있는 최소 검증 명령이어야 합니다.
...
| 현재 상태를 보고 싶다 | `/dashboard` | 현재 상태 확인 |
...
```bash
./.ai/scripts/check-dangerous-command.sh "<command>"
./.ai/scripts/record-retry.sh <signature>
./.ai/scripts/check-circuit-breaker.sh <signature>
./.ai/scripts/verify.sh
./.ai/scripts/dashboard.sh
```
...
- `check-dangerous-command.sh`  
  위험한 명령 실행 전 차단
- `record-retry.sh`  
  실패 기록 + circuit breaker 판단에 필요한 데이터 누적
...
2. `./.ai/scripts/sync-adapters.sh`
3. `./.ai/scripts/verify.sh`
