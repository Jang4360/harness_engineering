...
- For Codex implementation sessions, treat root `AGENTS.md` and `.agents/skills/` as the primary enforcement surface after installing the root entrypoints.
...
- Run `.ai/scripts/update-progress.sh` after changing item statuses in `progress.json` to keep summary counts in sync.
- Run `.ai/scripts/update-metrics.sh` after retry, promotion, blocker, or readiness state changes.
...
- After `plan-eng-review` or `autoplan`, run `.ai/scripts/check-plan-readiness.sh` on the updated plan artifact and keep refining until it passes or an external blocker is explicitly recorded.
