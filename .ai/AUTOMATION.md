...
- **Dangerous command guard**: `.ai/scripts/hook-pre-bash.sh` intercepts every Bash tool call via Claude Code `PreToolUse` hook (`.claude/settings.json`) and blocks destructive commands before execution.
...
- **Codex session preflight**: `.codex/hooks.json` wires a minimal `SessionStart` command that runs `.ai/scripts/codex-preflight.sh` to surface guard commands, progress state, repeated-failure warnings, and Claude review routing. It is advisory, not a replacement for tool-level blocking.
...
- **Adapter sync**: `.ai/scripts/sync-adapters.sh` copies canonical skills and runtime adapter templates into `.claude/`, `.agents/`, and `.codex/`; call this explicitly after canonical skill or adapter-template changes.
...
- **Promotion**: Call `.ai/scripts/record-promotion.sh` when a pattern is ready to be promoted. Invoke the `learn` skill to choose the right destination. This also refreshes derived metrics.
- **Scoring**: Call `.ai/scripts/score.sh` to get a readiness snapshot. Update `.ai/EVALS/metrics.json` after real sprint usage.
...
6. The pattern is promoted and `.ai/scripts/record-promotion.sh` logs the event.
